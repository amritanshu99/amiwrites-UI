...
function TaskManager() {
  ...
  const axiosAuth = axios.create({ baseURL: API_BASE, headers: { Authorization: `Bearer ${token}` } });
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e) => analyticsRef.current && !analyticsRef.current.contains(e.target) && setShowAnalytics(false);
  ...
  const fetchTasks = async (tokenToUse = token) => {
    try {
      setLoading(true);
      const res = await axios.get(API_BASE, { headers: { Authorization: `Bearer ${tokenToUse}` } });
      ...
      <div className="max-w-5xl mx-auto">
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-sm bg-black/30">
            <div className="flex flex-col items-center justify-center gap-4 bg-white dark:bg-zinc-900 shadow-2xl rounded-2xl px-8 py-6 w-80 animate-fade-in">
              ...
              <div className="text-base sm:text-lg font-semibold text-gray-700 dark:text-gray-200">Loading your mission...</div>
            </div>
          </div>
        )}
        <h1 className="text-5xl font-extrabold text-center mb-4 text-gray-900 dark:text-blue-200 drop-shadow tracking-tight"> Mission Control Dashboard</h1>
        ...
        {showAnalytics && (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4 sm:px-0">
            <div ref={analyticsRef} className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto relative shadow-2xl">
              <button onClick={() => setShowAnalytics(false)} className="absolute top-2 right-3 text-gray-500 hover:text-red-600 font-bold text-2xl">&times;</button>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-10">
              ...
              {editingTaskId ? (
                <div className="flex gap-2">
                  <button className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-xl font-semibold transition-all disabled:opacity-50" onClick={handleUpdateTask} disabled={loading}>Update</button>
                  <button className="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-xl font-semibold transition-all disabled:opacity-50" onClick={handleCancelEdit} disabled={loading}>Cancel</button>
                </div>
              ) : (
                <button className="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-xl font-semibold transition-all disabled:opacity-50" onClick={handleAddTask} disabled={loading}>Add</button>
              )}
            </div>
